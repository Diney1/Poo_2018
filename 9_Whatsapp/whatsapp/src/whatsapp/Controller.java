...
public class Controller {
	...
	public String query(String line) {
		...
		//convidar uma pessoa ao grupo
			...
			grupos.get(ui[2]).escrever(new Message(""+msg_indice, usuarios.get(ui[3]).getIdUSer(), "entrou no grupo"));
		}
		...
		//Sair do grupo
			...
			grupos.get(ui[2]).escrever(new Message(""+msg_indice, usuarios.get(ui[1]).getIdUSer(), "saiu"));
		}
		...
		//Envia uma nova mensagem para o grupo
			...
			for(int i = 3; i < ui.length; i++)
				texto += ui[i] + " ";
			grupos.get(ui[1]).escrever(new Message(""+msg_indice, usuarios.get(ui[2]).getIdUSer(), texto));
		}
		...
		//Mostra as mensagens que forma mandadas no grupo
		else if(ui[0].equals("msgchat")) {
			String msg = "";
			for(Message m : grupos.get(ui[1]).msg.getAll()) {
				msg += m + "\n";
			}
			return msg;
		}
